...
*
* fastDFS上传文件，上传库调用实践(上传文件file_buffer-生成动态链接库，供lua调用，并给lua返回相应文件上传结果)
...
int do_bf_upload(const char *conf_filename, const char *file_content, int64_t file_size, const char *file_ext_name, UploadFileInfo *upload_file_info)
{
     ...
     *group_name = '\0';
     ...
     // free(file_content);//一定不要手动释放已经添加到释放池中的对象,除非你用strong属性修饰对象的指针.
     ...
     strcpy(upload_file_info->ip_addr, file_info.source_ip_addr);
     strcpy(upload_file_info->remote_filename, remote_filename);
     upload_file_info->port = storageServer.port;
     upload_file_info->crc32 = file_info.crc32;
     upload_file_info->file_size = file_info.file_size;
...
{
     const char *conf_filename = luaL_checkstring(L,1);
     const char *file_content = luaL_checkstring(L,2);
     const char *file_ext_name = luaL_checkstring(L,3);
     int64_t file_size = luaL_checknumber(L,4);
     UploadFileInfo upload_file_info;

     do_bf_upload(conf_filename, file_content, file_size, file_ext_name, &upload_file_info);

     lua_pushstring(L, upload_file_info.remote_filename);
     lua_pushstring(L, upload_file_info.ip_addr);
     lua_pushnumber(L, upload_file_info.port);
     lua_pushnumber(L, upload_file_info.file_size);
